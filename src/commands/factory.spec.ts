  ...
  it('should return IgnoreCommand when ignore is specified', () => {
    expect(
      commandFactory
        ...
        .filter(x => x instanceof IgnoreCommand)
    ).toHaveLength(1)
  })
  it('should ommit IgnoreCommand when ignore is not specified', () => {
    expect(
      commandFactory.make({}).filter(x => x instanceof IgnoreCommand)
    ).toHaveLength(0)
  })
  it('should return GetFilesCommand command when foldersOnly is false', () => {
    expect(
      commandFactory
        ...
        .filter(x => x instanceof GetFilesCommand)
    ).toHaveLength(1)
  })
  ...
  it('should return GetFoldersCommand command when foldersOnly is true', () => {
    expect(
      commandFactory
        ...
        .filter(x => x instanceof GetFoldersCommand)
    ).toHaveLength(1)
  })
